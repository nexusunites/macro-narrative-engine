...
class NarrativeConstellationTests(unittest.TestCase):
    ...
    def test_attention_cloud_buttons_detail_and_reduced_motion_contract(self):
        ...
        self.assertIn("data-attention-cloud", template)
        self.assertIn("data-cloud-detail", template)
        self.assertIn("cloud-detail-highlight 600ms", css)
        self.assertIn(".cloud-chip:focus-visible", css)
    ...
    def test_populated_single_empty_and_malformed_contexts_render(self):
        ...
        populated = template.render(view={"cloud": dashboard.build_attention_cloud(populated_run())})
        self.assertNotIn("<svg", populated)
        self.assertIn("AI / Tech Growth", populated)
        self.assertIn("data-cloud-entry", populated)
        self.assertLessEqual(populated.count("data-cloud-entry="), 3)
        single = template.render(view={"cloud": dashboard.build_attention_cloud({"group_scores": {"Macro Pressure": 2}})})
        self.assertEqual(single.count("data-cloud-entry="), 1)
        empty = template.render(view={"cloud": dashboard.build_attention_cloud({"group_scores": {"broken": "nope"}})})
