...
| Network Confidence Model | **Implemented.** `mne/network_confidence.py` deterministically evaluates whether the Evidence Network is currently capable of supporting reliable macro understanding by synthesizing only existing persisted diagnostics: `network_health`, `source_confidence`, `source_reliability.summary`, and `coverage_intelligence`. It persists under `source_intelligence.network_confidence` on live runs, preserves honest `UNKNOWN` states for failed-headline or missing-input paths, and is displayed in the Admin Evidence Network section. It does not fetch data, rescore narratives, call LLMs, alter replay behavior, change source status, or represent investment confidence, prediction confidence, or narrative conviction. Operations Center rollup integration, Dashboard Trust Summary integration, broader user-facing display, and automated source quarantine remain future. | Catches the failure mode per-evidence validation cannot: clean collection from a dangerously thinned network | Done | Medium | Complete |
...
| Network Confidence Dashboard Trust Summary integration | Future, not implemented. Decide whether and how the user-facing Dashboard Trust Summary should reflect Network Confidence without exposing admin diagnostics or implying investment confidence, prediction confidence, or narrative conviction. | Gives users an appropriately scoped evidence-network adequacy signal if product design calls for it | Network Confidence Model (done), Dashboard Trust Summary (done), UX scope | Medium | Future |
...
5. Epic 7 / Epic 8: remaining source-intelligence and admin controls. Source Quarantine automation, Network Confidence Operations Center rollup integration, Network Confidence Dashboard Trust Summary integration, broader Network Confidence user-facing display, historical/skipped-run browsing, version comparison, and temporal integrity audit tooling remain future work. Source Confidence Model, Network Confidence Model, Source Reliability / Quarantine Tracking, Operations Center MVP, Historical Replay Admin Console, and Historical Research Workspace Integration are already complete.
