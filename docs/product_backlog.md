...
| Historical Evidence Backfill Architecture | **Documented.** `docs/historical_evidence_backfill_architecture.md` defines the architecture for on-demand historical evidence reconstruction: a Historical Source Registry model, backfill request model, connector contract, evidence normalization into standard `EvidenceObject`s tagged `evidence_origin = HISTORICAL_BACKFILL`, a three-tier storage model (Tier 2 - normalized evidence + replay output - as default), a backfill manifest/cache keyed by `backfill_id`, and an additive integration point with Historical Replay's existing evidence-selection/cutoff logic. Architecture only; no connectors, fetching, database, or UI were implemented. | Establishes how MNE can support historical dates beyond its live capture window without storing all history up front, before any implementation begins | Historical Replay foundation (done) | Medium | Complete |
| Historical Evidence Backfill MVP | Future, not implemented. Build the Historical Source Registry config/loader, the backfill request/manifest plumbing, and the additive Historical Replay evidence-source integration described in `docs/historical_evidence_backfill_architecture.md` (Section 15, steps 1, 3-4). | Makes on-demand historical reconstruction real for at least one source, proving the architecture before wider connector investment | Historical Evidence Backfill Architecture (done) | High | Future |
| Fed/FOMC Backfill MVP | Future, not implemented. Implement the first historical source connector against Fed/FOMC historical communications, per `docs/historical_evidence_backfill_architecture.md` Section 11 (public, structured, macro-relevant, avoids news/paywall complexity). | First real historical evidence beyond MNE's live capture window, feeding Historical Replay for pre-live dates | Historical Evidence Backfill MVP | High | Future |
| Historical Comparison View | Future, not implemented. A marketable, user-facing view comparing narrative reads across historical dates. Explicitly depends on Historical Evidence Backfill MVP shipping first - a comparison view is only honest and useful once real, cached, honestly-labeled historical coverage exists to compare against, per `docs/historical_evidence_backfill_architecture.md` Section 15, step 6. | Turns accumulated historical reconstruction into a marketable product surface | Historical Evidence Backfill MVP, Fed/FOMC Backfill MVP, Historical Research Workspace Integration (done) | High | Future |
...
| Historical evidence connectors | Future, not implemented. Archival ingestion needed for dates before live capture. Cross-listed with Epic 2; architecture documented in `docs/historical_evidence_backfill_architecture.md` (Historical Evidence Backfill Architecture, done), implementation remains future. | Enables Historical Replay beyond the current live window | Historical Replay foundation (Epic 2, done); Historical Evidence Backfill Architecture (Epic 2, done); fuller replay product maturity | High | Future |
...
2. Epic 2 / Epic 1: User-facing Historical Replay UI and standard `/research` historical mode remain future work. The admin-gated Historical Research Workspace Integration is already complete; future replay comparison / historical context views should be built only where backed by the implemented replay foundation, the completed admin console, and available historical evidence.
