...
- **Narrative Memory Foundation** — deterministic, live-run-based recent memory over meaningful runs only; persists `narrative_memory` on live runs, excludes replay artifacts, classifies theme/group memory states, emits `NARRATIVE_MEMORY` telemetry, and renders an admin-only Narrative Memory section.
...
**Current status:** **Narrative Memory Foundation implemented.** `mne/narrative_memory.py` builds deterministic memory from recent meaningful live runs only, persists `narrative_memory` on live runs, excludes replay artifacts as inputs, and excludes or handles failed/no-signal runs according to the meaningful-run policy. The foundation produces theme/group memory records with dominant, persistent, building, fading, recurring, re-accelerating, dormant, emerging, and absent states; streak, rank, score-delta, and share-delta metrics; deterministic template-based summaries; and thin-history warnings. Admin now renders an admin-only Narrative Memory section, and pipeline telemetry includes the `NARRATIVE_MEMORY` stage. This does not use AI/LLMs, does not generate essays or predictions, and is admin-visible only for now. The full Narrative Memory product is not complete: daily-only memory mode, user-facing Narrative Memory UI, Research Workspace memory integration, narrative essays / AI Analyst, predictive memory / forecast-like behavior, and memory based on historical replay timelines remain future work.

**Dependencies:** The completed foundation depends on recent meaningful live-run history only. Future user-facing memory, Research Workspace integration, daily-only memory mode, and replay-timeline memory depend on Historical Replay maturity and/or Research Workspace historical integration as needed.

**Epic priority:** **Partially implemented; remaining product surface deferred**
...
| User-facing Narrative Memory UI | Future, not implemented. Standard dashboard/research surfaces do not expose Narrative Memory yet. | Lets users inspect accumulated narrative memory directly | Narrative Memory Foundation (done), UX scope | Medium | Future |
| Research Workspace memory integration | Future, not implemented. Research Workspace does not yet consume Narrative Memory. | Adds accumulated context to narrative investigations | Narrative Memory Foundation (done), Research Workspace integration design | Medium | Future |
...
| Narrative essays / AI Analyst | Future, not implemented. Narrative Memory does not currently generate essays, AI analysis, or LLM-authored explanations. | Could make long-run memory more readable only under explicit AI governance | AI Experience, Research Workspace memory integration | High | Future |
...
7. Epic 3 / Epic 1: Narrative Memory Foundation is already complete. Remaining future work should start with Research Workspace Narrative Memory integration or user-facing Narrative Memory UI only after product behavior is deliberately scoped; historical replay timeline memory and comparison views remain future work and should wait for sufficient replay maturity and historical evidence.
...
- **Epic 3 (Narrative Memory)** foundation work is complete for recent meaningful live runs. Future replay-timeline memory, historical comparison views, and Research Workspace memory integration should wait until **Epic 2 (Historical Replay)** and the relevant product surface are mature enough for the specific memory work.
