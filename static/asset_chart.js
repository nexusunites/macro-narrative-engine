(function () {
  ...
  function render(range) {
    ...
    bars.forEach(function(bar,index){var events=view.events[bar.date];if(!events||!events.length)return;var cx=left+view.slot*(index+.5),above=yFor(bar.high)-18;
      var flipped=above<top+14||Math.abs(above-yHigh)<18,cy=flipped?yFor(bar.low)+18:above,glyph=events.length>1?events.length:(events[0].type==="earnings"?"E":"M");
  ...
  function openPanel(eventDate){var events=view.events[eventDate]||[];if(!events.length||!panel||!panelBody)return;panelBody.innerHTML=events.map(function(item){return '<article><span class="event-type">'+esc(item.type)+' · '+esc(item.date)+'</span><h3>'+esc(item.title)+'</h3><p>'+esc(item.blurb)+'</p><p>'+esc(item.detail)+'</p><p><span class="execution-label">'+esc(copy.event_source)+'</span><br>'+esc(item.source)+'</p></article>';}).join("");panel.hidden=false;if(chartMain)chartMain.classList.add("has-panel");}
