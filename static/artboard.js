(() => {
  ...
  board.nodes = Array.isArray(board.nodes) ? board.nodes.map(({id, kind, slug, x, y}) => ({id, kind, slug, x, y})) : [];
  ...
  const renderNode = node => {
    const data=storyData(node.slug), article=document.createElement("article");
    article.className=`studio-evidence direction-${data.direction}`;article.style.left=`${node.x}px`;article.style.top=`${node.y}px`;article.dataset.boardItem="";article.dataset.itemId=node.id;article.dataset.storySlug=node.slug;article.tabIndex=0;
  ...
  const openPanel = async slug => {
    ...
    try {
      ...
      const sections=[];
      if(data.headlines?.length) sections.push(`<section><h3>${copy.intelligence_headlines}</h3><ul>${data.headlines.map(x=>`<li>${escapeHtml(x.title)}${x.source?` <small>${escapeHtml(x.source)}</small>`:""}</li>`).join("")}</ul></section>`);
      ...
      if(data.catalyst) sections.push(`<section><h3>${copy.intelligence_catalyst}</h3><p>${escapeHtml(data.catalyst.name)} <small>${escapeHtml(data.catalyst.timing)}</small></p></section>`);
  ...
  const bindNode = el => {
    ...
    el.querySelector("[data-board-remove]").addEventListener("click",()=>{const id=el.dataset.itemId;board.nodes=board.nodes.filter(n=>n.id!==id);board.connections=board.connections.filter(c=>c.from!==id&&c.to!==id);el.remove();renderLinks();empty.classList.toggle("hidden",board.nodes.length>0);scheduleSave();});
