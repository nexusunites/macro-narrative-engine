...
def validate_sector_map(data: Any) -> SectorMapConfig:
    ...
    for narrative in sorted(raw_narratives):
        ...
        for index, item in enumerate(record["sectors"]):
            ...
            seen.add(sector)
            expected_expression = item.get("expected_expression")
            if expected_expression is not None and expected_expression not in {"UP", "DOWN"}:
                raise SectorIsolationError(f"{location}.expected_expression must be UP or DOWN")
            sectors.append(SectorMapping(sector, role, rationale, display_enabled, expected_expression))
...
def build_sector_isolation_context(narrative: str, config: SectorMapConfig | None = None, participation: dict | None = None) -> dict:
    ...
    for mapping in mappings:
        if not mapping.display_enabled:
            continue
        observed = ((participation or {}).get("sectors") or {}).get(mapping.sector, {})
        state = observed.get("participation_state", "UNAVAILABLE")
        freshness = observed.get("data_freshness", "UNAVAILABLE")
        instrument = observed.get("instrument")
        sectors.append({
            "sector_key": mapping.sector, "sector_name": SECTOR_KEYS[mapping.sector],
            "connection_role": mapping.role, "structural_role": mapping.role, "role_label": sector_isolation_copy(f"role_{mapping.role}"),
            "participation_state": state, "participation_label": sector_isolation_copy(observed.get("participation_label_key", f"participation_{state}")),
            "data_freshness": freshness, "freshness_label": sector_isolation_copy(observed.get("freshness_label_key", f"freshness_{freshness}")),
    ...
    participation_breadth = {**participation_breadth, "summary": sector_isolation_copy(f"breadth_{participation_breadth['state']}")}
    return {"narrative": narrative, "narrative_display_name": narrative, "sectors": sectors_tuple, "breadth": compute_sector_breadth(sectors_tuple), "participation_breadth": participation_breadth, "has_mapping": bool(sectors_tuple), "limitations": (sector_isolation_copy("curated_notice"), sector_isolation_copy("persisted_notice"), sector_isolation_copy("separate_notice"), sector_isolation_copy("unavailable_not_detached"), sector_isolation_copy("not_forecast"), sector_isolation_copy("calendar_limit"))}


def build_sector_isolation_preview(run: dict, dominant_narrative: str | None = None, limit: int = 4, participation: dict | None = None, config: SectorMapConfig | None = None) -> dict:
    ...
    context = build_sector_isolation_context(narrative, config, participation)
